...
"""
# Create a Blueprint for the submitter module
submitter_blueprint = Blueprint('submitter', __name__)
...
def load_yaml_data(file_path):
    ...
    try:
        with open(file_path, 'r', encoding="utf8") as file:
            ...
            current_app.logger.info(f"Loaded data from {file_path}: {data}")
        ...
        current_app.logger.error(f"Error loading YAML data from {file_path}: {e}")
    ...
    try:
        ...
        for yaml_file in Path(directory_path).glob('*.yml'):
            ...
            if yaml_data and 'resources' in yaml_data:
                ...
                current_app.logger.warning(f"No 'resources' key found in file {yaml_file} or file is empty.")
        current_app.logger.info(f"All content loaded: {resources}")
        return {'resources': resources}
    except Exception as e:
        current_app.logger.error(f"Error loading content from directory {directory_path}: {e}")
        return {'resources': []}  # Return empty list in case of error

@submitter_blueprint.route('/api/get_unique_values', methods=['GET'])
def get_unique_values_from_yamls():
    ...
    resources_dir = Path('/resources')  # Updated path for Docker environment
    current_app.logger.info(f"Loading resources from directory: {resources_dir}")
    
    content = all_content(resources_dir)
    
    if not content['resources']:
        current_app.logger.warning("No resources found in the YAML files.")
    ...
    current_app.logger.info(f"Unique Tags: {unique_tags}")
    current_app.logger.info(f"Unique Types: {unique_types}")
    current_app.logger.info(f"Unique Licenses: {unique_licenses}")
...
@submitter_blueprint.route('/api/get_yaml_files', methods=['GET'])
def get_yaml_files():
    """
    List YAML files in a directory.
    """
    resources_dir = Path('/resources')  # Updated path for Docker environment
    current_app.logger.info(f"Listing YAML files from directory: {resources_dir}")
    yaml_files = sorted([str(yaml_file.name) for yaml_file in resources_dir.glob('*.yml')])
    return jsonify(yaml_files)

@submitter_blueprint.route('/api/materials', methods=['GET'])
def get_materials():
    """
    Endpoint to fetch all materials.
    """
    resources_dir = Path('/resources')  # Updated path for Docker environment
    current_app.logger.info(f"Loading materials from directory: {resources_dir}")
...
@submitter_blueprint.route('/api/submit_material', methods=['POST'])
def submit_material():
    """
    ...
    try:
        ...
        current_app.logger.error(f"Error creating pull request: {e}")
